...
const products: string = window.localStorage.getItem("productList")
   ...
   <thead><tr>
    ...
    <th class="w-25">Price</th>
        ...
        for (let idx of cartItems) {
            ...
            if(count!=null){
                const productstring = `<tr><td ><a href="./product.html?id=${product?.id}"><img src ="${product?.image}" class="cartproductimg"></a></td>
                <td >${product?.title}</td>
                <td ><button type="button" class="minus btn btn-success" onClick = "decCount(${product?.id},${product?.price})">-</button>
                <button type="button" class="countvalue btn btn-dark"  id = "countval${product?.id}">${count}</button>
                <button type= "button" class="plus btn btn-success" onClick = "incCount(${product?.id},${product?.price})">+</button></td>
                <td id="totalPrice${product?.id}" class="w-25">${price}</td></tr>`;
    ...
    const totalPrice = document.getElementsByClassName("cartprice")[0];
        const totprice : string = `<h4>Total Price : ${totalcartitemsprice}</h4>
